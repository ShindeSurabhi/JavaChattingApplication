...
//Socket, that implements one side of a two-way connection.
//ServerSocket class, which implements a socket that servers can use to listen for and accept connections to clients
...
public class Server71 implements ActionListener{
	...
	static ServerSocket skt;  //declaration
 ...
 //dp 
        ImageIcon i4;
        i4 = new ImageIcon(ClassLoader.getSystemResource("chat/icons7/rose-modified.png"));
        Image i5;  
...
//        a1.setWrapStyleWord(true); //lines will be wrapped at word boundaries , if false then at character boundaries
 ...
 //SendButton
    ...
    //getContentPane().setBackground(new Color(4, 52, 107)); //set bg color of whole frame to yellow
	        ...
	        f1.setLocation(700,5);   //Sets location of frame 
	...
	//	a1.setText(a1.getText()+"\n\t\t\t"+ out); //setting dynamically text(old text of TextArea + text of TextField) on TextArea 
		...
		dout.writeUTF(out);//sending data after clicking on send button
...
public static void main(String[] args){
    ...
    try {
    	...
    	//Defining objects
    	skt = new ServerSocket(6001); //any port can be passed as long as it is not occupied
    	while(true) {
    	s = skt.accept(); //accept data
